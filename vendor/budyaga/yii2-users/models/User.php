...
class User extends ActiveRecord implements IdentityInterface
{
    ...
    const USER_TYPE_PEDIATR = 2;   
    const USER_TYPE_NEVROPATOLOG = 3;
    const USER_TYPE_OVTALMOLOG = 4;
    const USER_TYPE_TRAVMOTOLOG = 5;   
    const USER_TYPE_USER = 6;
    ...
    {
        return [
            self::SEX_MALE => Yii::t('users', 'Мужчина'),
            self::SEX_FEMALE => Yii::t('users', 'Женщина'),
        ];
    }
    ...
    {
        return [
            self::USER_TYPE_ADMIN => Yii::t('users', 'Администратор'),
            self::USER_TYPE_PEDIATR => Yii::t('users', 'Педиатр'),
            self::USER_TYPE_NEVROPATOLOG => Yii::t('users', 'Невропатолог'),
            self::USER_TYPE_OVTALMOLOG => Yii::t('users', 'Офтальмолог'),
            self::USER_TYPE_TRAVMOTOLOG => Yii::t('users', 'Травмотолог'),
            self::USER_TYPE_USER => Yii::t('users', 'Пациент'),
        ];
    }
    ...
    {
        return [
            self::STATUS_NEW => Yii::t('users', 'Новый'),
            self::STATUS_ACTIVE => Yii::t('users', 'Активный'),
            self::STATUS_BLOCKED => Yii::t('users', 'Блокированный'),
        ];
    }
    ...
    public function rules()
    {
        return [
            ...
            [['complaints', 's_history', 'diagnosis','diag_nevrop','diag_ovtal', 'diag_travmat', 'med_report'], 'string', 'max' => 65000],
            [['created_at', 'updated_at', 'status', 'sex', 'user_type'], 'integer'],
            ['status', 'default', 'value' => self::STATUS_ACTIVE],
            ['status', 'in', 'range' => [self::STATUS_NEW, self::STATUS_ACTIVE, self::STATUS_BLOCKED]],
             ['user_type', 'in', 'range' => [
                self::USER_TYPE_ADMIN, 
                self::USER_TYPE_PEDIATR,
                self::USER_TYPE_NEVROPATOLOG, 
                self::USER_TYPE_OVTALMOLOG, 
                self::USER_TYPE_TRAVMOTOLOG,
                self::USER_TYPE_USER,]],
    ...
    public function attributeLabels()
    {
        return [
            ...
            'created_at' => Yii::t('users', 'Дата записи'),
            'updated_at' => Yii::t('users', 'Дата обновления'),
            'complaints' => Yii::t('users', 'Жалобы'),
            's_history' => Yii::t('users', 'Краткий анамнез, объективный статус'),
            'diagnosis' => Yii::t('users', 'Диагноз педиатра'),
            'diag_nevrop' => Yii::t('users', 'Диагноз невропатолога'),
            'diag_ovtal' => Yii::t('users', 'Диагноз офтальмолога'),
            'diag_travmat' => Yii::t('users', 'Диагноз травмотолога-ортопеда'),
            'med_report' => Yii::t('users', 'Медицинское заключение'),
