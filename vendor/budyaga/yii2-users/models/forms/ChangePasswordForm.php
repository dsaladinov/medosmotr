...
class ChangePasswordForm extends Model
{
    ...
    {
        return [
            'old_password' => Yii::t('users', 'Старый пароль'),
            'new_password' => Yii::t('users', 'Новый пароль'),
            'new_password_repeat' => Yii::t('users', 'Повторите новый пароль'),
        ];
    }
    ...
    public function validatePassword($attribute, $params)
    {
        ...
        if (!$this->hasErrors() && $user->password_hash != '') {
            if (!$user->validatePassword($this->old_password)) {
                $this->addError($attribute, Yii::t('users', 'Старый пароль неправильный'));
            }
        }
    }
